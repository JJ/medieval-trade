...
\item Validate the dataset by recurring to internal checks or checks against another existing dataset for the same era.
...
\item Analyze data before and after the changepoint to establish the factors that contributed to it. Use again statistical analysis for doing it, from complex network analysis to other kinds of methods.
