...
One of the issues with a statistical approach to the study of history is the nonexistence of a methodology that goes back from the statistically established facts to the historical facts they are supposed to prove. In this paper we adapt the methodology we used in \citep{changepoint.statphys} to the subset of data referring to the Iberian peninsula.
