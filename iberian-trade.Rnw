...
<<trade.stationarity.test, echo=FALSE, message=FALSE>>=
library(tseries)
...
A visual inspection of the time series can be used to check it against known historical events; the fact that they appear in the series with a plausible impact would be a source of additional validation.

<<trade.events, echo=FALSE, fig.height=4, fig.cap="Time series of interior and all trade link density for the Iberian Peninsula, with known historical events indicated.", fig.pos="h!tb">>=
ggplot(links_time_series, aes(x=year)) +
  geom_line(aes(y=interior_links, color="Interior links"), linewidth=1) +
  geom_line(aes(y=exterior_links, color="All links"), linewidth=1) +
  scale_color_manual(values=c("Interior links"="#E69F00", "All links"="#56B4E9")) +
  labs(title="Trade link density in the Iberian Peninsula",
       ...
       y="Trade link density") +
  theme_minimal() +
  theme(legend.title = element_blank())+ # vertical dashed line year = 340 labeled "Restoration Emerita Augusta"
  geom_vline(xintercept=340, linetype="dashed", color="red") +
  annotate("text", x=336, y=max(links_time_series$exterior_links)*0.9, label="340: Restoration Emeriga Augusta", color="red", angle=90, vjust=-0.5) +
  geom_vline(xintercept=385, linetype="dashed", color="blue") +
  annotate("text", x=383, y=max(links_time_series$exterior_links)*0.9, label="385: End of Priscillianism", color="blue", angle=90, vjust=-0.5) +
  geom_vline(xintercept=409, linetype="dashed", color="purple") +
  annotate("text", x=407, y=max(links_time_series$exterior_links)*0.9, label="409: Vandal invasion", color="purple", angle=90, vjust=-0.5)
...
changepoint_dates_found <- data.frame(
  Method = c("All links, cpt.meanvar (+filtered)", "All links, cpt.meanvar (midpoint)","All links, Lanzante/Pettitt", "All links, Lanzante/Pettitt (filtered)", "Internal links, cpt.mean (+filtered)", "Multi-sequence, e.divisive (+filtered)","Multi-sequence, e.divisive (midpoint)"),
