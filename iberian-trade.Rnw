...
<<trade.link.evolution, echo=F, message=F, warning=F, fig.cap="Evolution through time of the average trade link density by decade with the main trading partners of the Iberian Peninsula.", fig.height=4>>=
library(purrr)
...
top_links <- paste0( top_edges$origin, " - ", top_edges$destination)
