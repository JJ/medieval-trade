...
<<trade.changepoint, echo=FALSE, fig.height=4, fig.cap="Changepoint in the time series of Iberian trade link density, with mean before and after the changepoint in red. This changepoint has been established following the cpt.meanvar method.", message=FALSE, fig.pos="h!tb">>=
library(changepoint)
...
Table \ref{tab:trade.changepoints.found} summarizes the changepoints found using the different methods described above, including results with and without filtering hoards with a long date range. Except for the Lanzante/Pettitt tests, there is no change in the dates found by the different changepoint detection methods. It is specially interesting to find no change in the {\sf e.divisive} method, which being a multi-sequence method, is the most robust of all.
