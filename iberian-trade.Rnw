...
As it can be observed, there is a statistically significant changepoint in the year \Sexpr{links_changepoint_date[1]}. This is roughly 20 years after the year found in \cite{changepoint.statphys}, but still almost 40 years short of the Fall of the Roman Empire with the ousting of the last emperor by Odoacre \cite{Chapter1WhenDidtheWestRomanEmpireFall}.
