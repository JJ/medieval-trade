...
\item {\em RQ2: Is there a change in the network of trade links that can explain the changepoint?} The main change is that the volume of trade links with Britannia drops abruptly after that changepoint, as can be seen in Figures \ref{fig:trade.link.evolution}, \ref{trade.link.evolution.detail} and \ref{fig:<trade.link.evolution.boxplot}. Although other nodes have a significant reduction too, and overall volume drops, the factor that contributed the most to the changepoint seems to be the almost total elimination of the node network in Britannia.
\end{itemize}
}
