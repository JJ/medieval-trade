...
\item {\em RQ1: Is there a changepoint in the Iberian Peninsula trade link time series?} Yes, using single-sequence and multi-sequence changepoint detection methods, as well as different interval censoring methods, we have {\em always} found statistically significant changepoints, which we have fixed at \Sexpr{multi_changepoint_year} since it is the most robust method, that uses all available data, and it is approximately in the middle of the different changepoints found through other methods.
\end{itemize}
}
