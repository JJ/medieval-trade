...
kable(all_links_changepoint_stats, caption="Changepoint detection statistics for the Iberian trade links time series. p < 0.05 in all cases. Check text for reference on the last column.", col.names=c("Method", "Changepoint year", "Changepoint (midpoint method)")) %>%
  kable_styling(full_width = F, position = "center") %>%
...
ggplot(regional_links_pasted_top, aes( x=cs, y=value, color=relative_to_changepoint))+
  ...
  labs( x="Trade link",y="Link density") +
