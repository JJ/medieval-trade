...
\rev{We should probably consider also the fact that, in some cases, imitative currency, that is, currency that simply follows a pre-existing pattern, but is not indeed minted by the original authority, was in circulation in the territories abandoned by Roman authorities. As far as we can tell, the FLAME database will not identify the original, but the imitative currency mint, ths taking into account the correct time frame where it was minted. This identification itself takes into account the absence of mints in some territories; they will simply not appear as the origin of coins in the database, thus not biasing the result. In general, FLAME database will only include data on hoards that are properly identified by a publication, as indicated in \cite{FLAME:bias}.}
