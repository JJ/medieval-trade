...
The FLAME database includes two fields with the start and end date estimated (and published) for the coin hoard\rev{; we will need to process these ranges into a time series so that we assign a specific value to every year in the series, which is what every time-series algorithm needs.
