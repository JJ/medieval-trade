...
\rev{The rest of the methodology would be largely qualitative, and to a certain extent speculative, since it would use the statistical analysis to create a narrative that would go from different factors to the changepoint, and from this one to the period boundary we are interested in, that from Antiquity to the Middle ages. }
