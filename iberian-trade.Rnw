...
\section{\rev{Research background}}
...
Focusing on the Iberian Peninsula, the Visigoth invasion did produce some discontinuity, but trade continued decreasing during the 5th and 6th centuries, although most changes were relatively slow \citep{fernandez2017aristocrats}. \rev{It is evident, as mentioned by \cite{heather2005fall}, that the invasions from peoples coming from beyond the Rhine, including Sueves and Vandals, caused major disruptions in the Iberian Peninsula, as reported by a chronicler called Hydatius. Heather writes (p. 209, op. cit.):}
...
\rev{It has been argued, however, that this {\em breakdown of frontier security} was not so much the cause of the invasion as a consequence of economic collapse and exhaustion of the elites that occurred in the Iberian Peninsula during the 4th century \cite{arce2007barbaros}. }
...
We might wonder about the validity of this dataset as an, if not complete, at least unbiased representative of the activity we want to study. Any archaeological dataset is a sample of all possible data \edit{on the objects that are considered in it}, and will probably have some biases, as revealed by \cite{rademacher2024data}. Certainly, those countries that are more urbanized will have found more coin hoards than those that are sparsely populated. In that paper, Spain has 36\% of sites with a numismatic rating of "Good" or better, over 461 hoards that have been found; it is among the 4 countries that have been described in caption to Table 1 of that paper as having good data quality; in the conclusion \rev{(p. 22 of the downloaded paper)}, it affirms that
\begin{quote}
[...] in Portugal and Spain urban hoards more often have a high data quality.
\end{quote}

So, this gives the subset of the original dataset we are using enough validity to be able to draw some conclusions from its statistical analysis\edit{; however, this matter is examined in more depth in the following subsections}. In what follows, we will describe which specific files we have used from that dataset, and the processing they have undergone (Subsection \ref{ssec:processing}), show a general overview of coin hoard data and its distribution in Subsection \ref{ssec:overview} and finally indicate how the dataset is validated in Subsection \ref{ssec:validation}. \rev{Please check the Data Availability section \ref{sec:data} for the original as well as processed datasets, along with the R code used to process them.}
...
We have {\em geolocated} and assigned the mints and hoards to a specific current territory, using the GeoApify service. These are geographical markings more than actual polities; in many cases, besides, the minting place is not exactly known, in which case the database assigns it to the conventional center of a current territory. \rev{In the case the database does not identify the minting place, that coin group is dropped; for instance, we would drop the first coin group above for the purposes of this paper.}

\rev{To make the historical analysis easier, these current territories have been converted to late Roman-early Medieval geographical entities in the cases the translation was direct or easier. The conversions we have performed are as follows:\begin{itemize}
...
\rev{Please note that there is no direction in these links; for convenience the pairs are stored in alphabetical order, but edges are undirected.}
...
\rev{This would answer the two first research questions:\begin{itemize}
...
\rev{This paper makes a methodological contribution to the field of digital humanities and historical studies from the point of view of complex systems.
