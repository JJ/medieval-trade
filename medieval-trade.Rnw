...
<<statphys.bw.before.after, warning=F, echo=FALSE, message=F, fig.height=4, fig.pos="h!tbp", fig.cap="Entropy of the trade network before and after the changepoint">>=
data.frame(
    node=V(all_links_after_network)$name,
    betweenness=V(all_links_after_network)$betweenness
  ) %>%
...
ggplot(comparison.table, aes(x=1:nrow(comparison.table))) +
  ...
  theme_minimal() +
  ggtitle("Betweenness Centrality Before and After Changepoint") +
