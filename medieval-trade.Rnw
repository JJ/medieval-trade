...
From this initial dataset, a new dataset with the number of links per year has been created, by simply adding up the number of pairs of places that appear for every year.

<<statphys.changepoint, echo=FALSE, message=F, fig.height=4, fig.pos="h">>=
library(dplyr)
library(tidyr)
...
plot(link.changepoint,xaxt = 'n', xlab="", ylab="", main="Number of links per year and mean before/after changepoint")
...
<<statphys.graph, echo=FALSE, message=F, fig.height=4, fig.pos="h">>=
library(dplyr)
all_links <- read.csv("data/all-links.csv", sep=";")
